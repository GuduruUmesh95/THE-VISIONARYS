...
import { Cpu, ShieldCheck, Landmark, Cloud, Network, Compass, Server, BarChart3, ChevronLeft, ChevronRight, X, CheckCircle2, ArrowRight } from "lucide-react";

export default function SolutionsBentoDrawer() {
  ...
  return (
    <section id="option1" className="w-full pt-24 md:pt-36 pb-6 md:pb-8 bg-transparent relative z-10 scroll-mt-[80px]">
      <style>{`
        .solutions-track {
          --card-width: 100%;
          --card-gap: 1.5rem;
      ...
      <div className="max-w-[85vw] xl:max-w-[80vw] mx-auto px-4 relative">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16">
          <div className="max-w-2xl">
            <span className="px-3 py-1 rounded-full bg-blue-500/20 text-blue-400 border border-blue-500/30 text-[10px] font-mono font-bold tracking-[0.3em] uppercase block mb-3 w-max">
              Option 1: Side Drawer
            </span>
            ...
            {pillars.map((pillar, index) => {
              ...
              return (
                ...
                >
                  ...
                  <button 
                    onClick={() => setActiveDrawer(pillar)}
      ...
      {/* Slide Drawer Overlay - Rendered at root level to fix z-index stacking issues */}
      {typeof document !== 'undefined' && createPortal(
        <div 
          className={`fixed inset-0 z-[9999] transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] ${activeDrawer ? "pointer-events-auto" : "pointer-events-none"}`}
        >
          {/* Dark Backdrop */}
          <div 
            className={`absolute inset-0 bg-black/30 backdrop-blur-md transition-opacity duration-700 ${activeDrawer ? "opacity-100" : "opacity-0"}`} 
            onClick={() => setActiveDrawer(null)}
          />
          
          {/* Right Drawer Panel */}
          <div 
            className={`absolute top-0 right-0 bottom-0 w-full md:w-[600px] bg-[#08080a] border-l border-[#FFB84D]/60 shadow-[0_0_100px_rgba(255,184,77,0.25),-15px_0_40px_rgba(255,184,77,0.15)] transition-transform duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] flex flex-col ${activeDrawer ? "translate-x-0" : "translate-x-full"}`}
          >
            {activeDrawer && (
              <>
                {/* Drawer Header */}
                <div className="flex items-center justify-between p-8 border-b border-white/5">
                  <div className="flex items-center gap-4">
                    <div className="p-3 bg-white/5 border border-white/10 rounded-xl">
                      <activeDrawer.icon className="w-6 h-6" style={{ color: activeDrawer.accent }} />
                    </div>
                    <div>
                  ...
                  <button 
                    onClick={() => setActiveDrawer(null)}
                    className="p-3 rounded-full hover:bg-white/5 text-zinc-400 hover:text-white transition-colors"
                  >
                    <X className="w-6 h-6" />
                  </button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-8 relative">
                  ...
                  <img 
                    src={activeDrawer.imgSrc} 
                    alt={activeDrawer.title} 
                    className="w-full h-48 object-cover rounded-2xl border border-white/10 mb-8"
                  />
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-10">
                    {activeDrawer.features.map((feat, i) => (
                      <div key={i} className="flex items-start gap-3 p-4 rounded-xl bg-white/[0.02] border border-white/[0.05]">
                        <CheckCircle2 className="w-5 h-5 shrink-0 mt-0.5" style={{ color: activeDrawer.accent }} />
                        <span className="text-zinc-200 text-sm font-medium">{feat}</span>
                      </div>
                    ))}
                  </div>

                  <div className="p-6 rounded-2xl border border-[#FFB84D]/30 bg-[#FFB84D]/5 relative overflow-hidden mt-8">
                    ...
                    <a href="#contact" onClick={() => setActiveDrawer(null)} className="relative z-10 inline-flex items-center gap-2 px-6 py-3 bg-white text-black font-bold text-xs uppercase tracking-widest rounded-xl hover:scale-105 transition-transform">
